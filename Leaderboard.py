...
class LeaderBoard:
    ...
    def del_linha(self,grid):
        ...
        for y in range(
            len(grid)
        ):  # Verifica se a linha esta cheia, se estiver adiciona na lista para deletar
        ...
        for y in linhas_deletar:
            ...
            grid.insert(
                0, [8] + [0] * 12 + [8]
            )  # Deleta as linhas cheias e adiciona uma nova linha vazia
            pontos += 100
    ...
    def salva_leaderboard(self,nome, pontuacao):
        leaderboard = self.carrega_leaderboard()
        leaderboard.append((nome, pontuacao))
        leaderboard = sorted(leaderboard, key=lambda x: x[1], reverse=True)[
            :5
        ]  # Seleciona apenas as 5 maiores pontuações no leaderboard e coloca elas no arquivo do LEADERBOARD
        with open(LEADERBOARD_FILE, "w") as f:
            for entry in leaderboard:
                f.write(f"{entry[0]},{entry[1]},")

    
    def render_text(self,texto, fonte, cor, superficie, x, y, centro=False):
        lines = texto.split("\n")
